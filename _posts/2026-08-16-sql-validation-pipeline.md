---
layout: post
title: "[개발노트] SQL Validation Pipeline"
sticky: false
hidden: true
---
 LLM이 생성한 SQL을 실행해 보면 정답이 아닌 경우가 있다. 이는 결국 잘못된 차트가 그려진다는 의미다. LLM은 태생적으로 확률적(stochastic)이고 비결정적(non-deterministic)이기 때문에, 똑같은 질문을 던져도 틀린 결과를 내는 SQL을 반환하는 경우가 발생한다. Gemini, Anthropic, OpenAI 등 여러 모델을 대상으로 Text-to-SQL 정확도, 응답 속도, 가격 등을 벤치마크하여 최적의 LLM을 선정했음에도 실제 결과는 실망스러웠다.
 
LLM을 믿고 SQL를 그대로 받아 먹는 구조에서 → LLM을 사용하되 그 결과를 독립적으로 검증하는 구조로 완전히 프로젝트 방향을 바꿨다. 개발이 아니라 실험이 되었다.
 
 아래에 Claude Code가 작성한 `SQL_VALIDATION_PIPELINE.md` 파일을 공유한다. 이 파일은 본 프로젝트에서 가장 핵심적인 문서이며, 현재도 계속 업데이트되고 있는 워킹(working) 문서다. 나는 이 문서를 기반으로 Claude Code와 소통하며 정확도를 개선해 나가고 있다.

---------------------------------------------------------
## SQL 검증 파이프라인

최종 수정 2026-09-15 12:55 KST
...
관련 코드: `app.py`의 `_cached_generate` → `_vet_generated_sql` → (`_clean_generated`, `validate_sql`, 재생성 repair, `_reroll_if_wrong`).

> **용어 규칙** — 세 단계의 동사를 일부러 분리했다. **clean**(정제) · **repair**(재생성) · **re-roll**(실행 후 재요청). 코드·로그·grep 어디서도 안 섞이게. "repair"는 오직 B(재생성)만 뜻한다.
> 
> **린트는 A도 B도 아니다** — 세 안전망 사이에 낀 **게이트** `validate_sql`의 일부다. 안전망이 _하는 일_(고친다/다시 묻는다)이라면, 게이트는 _판정_(통과냐 B로 보내냐)만 한다. 자세히는 아래 `validate_sql` 절을 참고.
...
|---|---|---|---|---|---|---|
|**A**|정제 (clean)|`_clean_generated` → `_clean_literals` · `_unqualify_order_by_aliases`|깨진 문자열 리터럴 (`'\n이숭용'`, NFD 한글, `\uXXXX` 잔재), 그리고 `ORDER BY`가 자기 출력 별칭에 붙인 CTE 한정자|코드가 조용히 손봄|❌ 안 함|해당 없음 — 재요청 자체가 없음|
|**B**|재생성 (repair)|인라인 (`_generate_in_thread(repair=…)`)|**SQL이 안 돎** — 문법·컬럼 오류, 린트 위반|오류 메시지를 LLM에 되먹여 재생성|✅ 재생성|**거부**|
|**C**|실행 후 검사 (re-roll)|`_reroll_if_wrong`|**값이 틀림** — 불가능 rate·통산 카운팅 초과·시즌별 차등 불일치, 그리고 **명단이 틀림** — 손잡이 필터 누출 (빈 결과는 트리거 아님)|재요청. 가드가 재검증할 수 있으면 진단값을 알려주고, 없으면 질문만|✅ 재요청 (단, 값 문제인데 5,000행 초과면 **안 부르고 바로 거부**)|**값이 틀리면 거부 · 손잡이 누출과 통산 rate 차등은 그대로 응답**|
...
**핵심 구분** — B는 _"실행 가능성"_(SQL이 도는가), C는 _"값 정확성"_(값이 맞는가)을 본다. A는 이 둘이 구조적으로 못 잡는 것들을 앞단에서 조용히 씻어낸다.

**A가 고치고 B가 되묻는 경계는 "고칠 방법이 하나뿐임을 증명할 수 있는가"다.** 증명되면 A가 고치고 (LLM 호출 0회), 안 되면 B가 되묻는다. 예를 들어 `ORDER BY c.player_label`은 그 이름이 CTE의 컬럼이 아니고 바깥 SELECT가 만든 별칭이므로 한정자를 뗀 읽기 말고는 성립할 수가 없다 — A의 몫이다. 반면 연결되지 않은 CTE는 어디에 JOIN해야 하는지 코드가 알 수 없어 린트가 판정하고 B가 되묻는다.

> **통산 rate 차등이 2026-09-15에 C에서 빠졌다.** 검사는 그대로 돌고 로그에 줄을 남기지만 차트를 뺏지 않고 재요청도 사지 않는다. 7주치 자기 로그가 근거다 — 18번 발화해 **18번 다 오탐**이었고, `Re-roll recovered a clean result`는 그 18번에 걸쳐 **0**이었다. 원인이 하나로 모인다: **질문이 모집단을 좁혔다.** 잣대 SQL은 「모든 시즌·모든 팀 통산」을 계산하는데 질문은 「어떤 모집단 안에서」를 계산하므로, 좁히면 두 값은 설계상 달라진다. 지금까지 팀·나이·포지션·스탯 문턱·연도 창 다섯 축을 사고가 날 때마다 막아 왔는데, 그것은 사람이 질문을 좁히는 방법의 블랙리스트이고 수렴하지 않는다. 반면 카운팅 초과는 「부분이 전체보다 클 수 없다」는 산술이라 모집단과 무관하므로 그대로 거부한다.
...
G1 -->|SQL 있음| A["<b>A. clean</b> — 리터럴 · ORDER BY 한정자<br/>코드가 조용히 고침 · LLM 안 부름"]
...
### A. 정제 (clean) — `_clean_generated` (LLM 응답마다 실행)
...
**씻는 것이 둘로 늘었다 (2026-08-20).** 리터럴에 이어 `ORDER BY`가 자기 출력 별칭에 붙인 CTE 한정자를 뗀다:

```sql
SELECT c.Player || ' #' || CAST(c.Id AS VARCHAR) AS player_label, c.ERA_plus
FROM career_era_plus c ORDER BY c.ERA_plus DESC, c.player_label
                                                 ^^ CTE에 없는 컬럼 — Binder Error
```

`player_label`은 바깥 SELECT가 만든 이름이라 CTE의 컬럼이 되지 않는다. 프롬프트가 이 실수를 그 이름 그대로 금지하는데도 2026-08-20 로그에서 2,640콜 중 64번 일어나 **repair를 부른 원인 중 단일 최대**였다(147회 중 61회, 41%). 명시적 규칙이 안 지켜지면 문장을 더 쓸 자리가 아니라 기계적으로 처리할 자리다.

증명 가능할 때만 고친다 — 한정자가 CTE를 가리키고, 그 CTE의 출력 컬럼 목록을 알 수 있고, 거기에 그 이름이 없고, 이 SELECT가 그 이름을 출력 별칭으로 만들 때. 원본 테이블 한정자, `SELECT *` CTE, 파싱 실패는 손대지 않는다. **`ORDER BY`에만 적용한다**: 같은 실수가 `WHERE`(19건)와 `SELECT`(7건)에도 나타나지만, 거기서 한정자를 떼면 이름이 다른 관계로 해석될 수 있어 질문의 답 자체가 바뀐다. `ORDER BY`에서는 별칭이 이미 스코프에 있고 한정된 형태가 아예 해석되지 않으므로, 뗀 쪽이 유일하게 의도 가능한 읽기다.

교정된 SQL은 **응답에 실려 캐시·화면·공유 토큰으로 간다.** 실행 시점에만 고치면 차트는 그려지지만 "이 차트의 SQL 보기"에 뜨는 쿼리가 남의 DuckDB에서 돌지 않는다 — 그 화면의 목적이 읽고 복사해 직접 돌려보는 것이므로, 이 앱 안에서만 도는 SQL은 그 일을 못 한다.
...
`queries.py`의 `validate_sql`은 세 검사를 **순서대로** 돌리고, 하나라도 걸리면 그 메시지를 반환한다 (반환값 = 그대로 B가 LLM에 되먹이는 오류 텍스트):

|#|검사|무엇을 보나|걸렸을 때|
|---|---|---|---|
|1|`_blocked_reason`|SELECT가 아닌 SQL (INSERT·DROP 등)|즉시 거부|
|2|**`lint_sql`**|**증명 가능하게 틀린 구조** — SLG 빠진 OPS, 리그 기준선 필터 누출, raw rate 컬럼 직접 사용, 매크로 인자 순서, 연결되지 않은 CTE, 반올림 별칭 정렬, 명시된 최소 기준 무시 등|→ **B**|
|3|`_run_sql` (DuckDB)|문법·컬럼 오류 — 실제로 한 번 실행해봄 (`execute_query`와 **같은 정제를 거친 뒤**)|→ **B**|
...
- 3번은 `execute_query`가 적용하는 것과 **같은 정제를 거친 SQL**을 돌린다. 원본 그대로 검증하면 곧 잘 돌아갈 쿼리에서 실패해, 정제가 이미 고친 것을 고치려고 repair를 사게 된다.

#### 린트 ≠ clean — 헷갈리기 쉬운 지점

||**A. clean** (`_clean_generated`)|**린트** (`lint_sql`)|
|---|---|---|
|어디|안전망 A|게이트 `validate_sql` 안 (2번)|
|무엇을|깨진 **문자열 리터럴** (`'\n이숭용'`, NFD 한글)과 **고칠 방법이 하나뿐인 구조 오류** (`ORDER BY c.player_label`)|증명 가능하게 틀렸지만 **옳은 형태가 하나로 정해지지 않는** SQL 구조|
...
린트와 repair 말고, **LLM에게 다시 묻지 않고 코드가 SQL을 직접 고쳐 실행하는** 길이 있다. 비용이 0이라 쓸 수 있으면 늘 이쪽이 낫고, 쓸 수 있는 조건은 하나다 — **고칠 방법이 하나뿐임을 증명할 수 있는가.**

|수단|하는 일|비용|쓰는 조건|
|---|---|---|---|
|린트|실행 전에 **거부**하고 B로 넘김|LLM 호출 1회|무엇이 옳은지 코드가 정할 수 없을 때|
|**재작성**|코드가 **직접 고쳐서** 실행|0|옳은 형태가 유일하게 결정될 때|
|repair (B)|DuckDB 오류를 되먹여 다시 받음|LLM 호출 1회|위 둘이 못 잡은 나머지|

재작성은 두 층에 있다. `ORDER BY` 한정자 교정은 **A**에서 일어나 응답에 실린다(캐시·화면·공유 토큰이 그 SQL을 담는다). 반면 `_inject_tiebreak`와 `_drop_artifacts`는 `execute_query` 안, **실행 시점**에만 산다.

둘을 가르는 기준은 **그것 없이 쿼리가 도는가**다. 한정자 교정은 없으면 아예 안 돌므로 저장본에 들어가야 하고, 동점 처리는 없어도 돌기 때문에(경계만 흔들린다) 들어가면 안 된다 — 읽으라고 만든 SQL에 숨은 컬럼과 서수 정렬 키가 섞이면 읽기 어려워진다.

#### 실행 시점 재작성 — `_inject_tiebreak`와 `_drop_artifacts`

`ROUND()`한 표시값으로 정렬하는 리더보드는 커트라인이 실행마다 흔들린다. 통산 OPS 9·10위인 나성범(0.914899)과 데이비스(0.915381)는 3자리로 둘 다 0.915라, `ORDER BY OPS DESC LIMIT 10`은 동점 처리 없이 아무나 10위로 올린다. `_inject_tiebreak`은 반올림 전 값을 CTE에 숨은 `<별칭>__rawtb` 컬럼으로 노출하고, 그 뒤에 이름 컬럼과 서수를 붙인다:

```sql
ORDER BY c.OPS DESC                                          -- LLM이 낸 것
ORDER BY c.OPS DESC, c.OPS__rawtb DESC, Player, 1            -- 실행되는 것
```

마지막 두 키는 값이 **진짜로 같을 때**를 위한 것이다. 최준석과 김재현은 통산 홈런이 둘 다 정확히 201이라 어떤 원시값으로도 못 가른다. 이름 컬럼(`player_label` → `Player`)이 먼저 갈라주고, 이름까지 같으면 첫 출력 컬럼의 서수 `1`이 받는다. **서수를 마지막에 두는 이유**는 팀별 집계처럼 이름 컬럼이 아예 없는 쿼리가 있기 때문이다 — 첫 출력 컬럼은 어떤 SELECT에나 정의상 존재한다.

숨은 컬럼은 결과가 `execute_query`를 떠날 때 `_drop_artifacts`가 떨어뜨린다. 정렬을 위한 실행 시점 인공물이지 데이터가 아니므로, 만드는 층과 없애는 층이 같은 것이 그 성격에 맞다. 이 처리를 SQL 쪽에서 회피하려던 시도가 한 번 있었고 — 쿼리가 `*`를 뽑으면 숨은 컬럼을 만들지 않는 방식 — 툴팁의 `OPS__rawtb: 0.9153...`은 사라졌지만 **원시값 정렬 키도 함께 사라져** 같은 리더보드가 나성범을 데이비스 위에 올렸다. 보이는 흠과 보이지 않는 오답을 맞바꾼 셈이라 되돌렸다.

재작성은 전부 best-effort다. 어느 쪽이든 DuckDB가 거부하는 SQL을 만들면 `execute_query`가 원본으로 되돌린다. **그 폴백이 조용하다는 것이 이 층의 함정이다** — 깨진 재작성은 재작성이 아예 없었던 것과 화면에서 구별되지 않으므로, 테스트는 재작성 결과가 **실제로 실행되는지**까지 단언해야 한다.

#### 세는 법 — 재작성은 2026-08-22까지 로그에 한 줄도 남기지 않았다

repair와 re-roll은 오래전부터 기록됐는데 재작성은 완전히 조용했고, 위의 폴백은 그래서 화면에서도 로그에서도 볼 수 없었다. 세 지점에서 남긴다.

|로그|어디서|레벨|언제|
|---|---|---|---|
|`rewrite unqualify`|`_clean_generated`|INFO|응답 하나당 한 번 (항상 `rw:fix`)|
|`rewrite tiebreak`|`validate_sql`|INFO|받아들인 SQL 하나당 한 번 (`raw` 포함이면 `rw:fix`, 아니면 `rw:tie`)|
|`rewrite FELL BACK`|`execute_query`|**WARNING**|폴백이 실제로 일어날 때|

**자리 선택이 곧 설계다.** tiebreak는 실행 시점 재작성이지만 `execute_query`는 질의 하나에 여러 번 돈다(검증 · 렌더 · 연도 슬라이더 비교). 거기서 찍으면 질의가 아니라 **실행 횟수**를 세게 되므로, 받아들여진 SQL 하나당 한 번 도는 `validate_sql`에서 예고한다 — `_inject_tiebreak`은 텍스트만 보는 순수 함수라 예고와 실제가 같다.

무엇이 붙었는지는 `raw+Player+ordinal`처럼 요약한다: `raw`는 반올림 전 값, 그다음이 이름 컬럼, 마지막이 서수다. 재작성된 SQL 전체를 남기면 한 줄이 수백 자가 되는데, 나중에 통계에 필요한 것은 어떤 **종류**의 키가 붙었나이지 쿼리 원문이 아니다.

`scripts/pipeline_health.py`가 이 줄들을 날짜별로 센다. **열마다 좋은 방향이 다르다** — `rw:fix`는 높을수록 싸고(LLM 호출을 아꼈다), `fallbk`와 `clean`은 0이 정상이며, `reroll`·`repair`·`refuse`는 낮을수록 좋다.

**재작성을 `rw:fix`와 `rw:tie`로 가르는 기준은 원본이 틀렸느냐다.** 요약에 `raw`가 있으면 원본이 `ROUND()`한 값으로 순위를 매기고 있었다는 뜻이고 — 표시상 같은 0.915에서 10위가 실제로 바뀐다 — `unqualify`는 DuckDB가 거부할 SQL이었다. 둘 다 `rw:fix`다. `raw`가 없는 tiebreak는 순위 자체는 맞고 동률 경계만 느슨했던 경우라 `rw:tie`로 가며, **그중 일부는 아예 무해하다**: 정렬 키가 GROUP BY 키와 같은 연도별 추이 같은 쿼리는 값이 유일해 동률이 불가능한데도 키가 하나라는 이유로 서수를 받는다.

그 무해한 경우를 재작성기에서 빼지 않은 것은 의도다. 결과가 한 행도 달라지지 않고(실측) 저장본에도 안 남으므로 손해가 **통계 하나뿐**인데, 빼려면 "정렬 키가 GROUP BY 키면 유일하다"를 증명하는 분기를 새로 만들어야 한다. 같은 종류의 영리한 분기가 2026-08-20에 원시값 정렬 키를 함께 없애 10위를 오답으로 바꾼 적이 있다. 통계는 열을 나눠 읽는 쪽이 싸다.

계측이 켜진 첫 harness (156질의): `rw:fix` **14.7%** · `rw:tie` 26.3% · `repair` **2.0%** · 나머지 0. **repair가 역사적 4.6%에서 절반 이하로 내려왔다** — 2026-08-20에 로그로 계산한 "repair 원인의 41%가 ORDER BY 한정자"라는 예측이 실측으로 확인된 셈이다.

#### 거부하지 않는 검출기 — `_unranked_limit`

`SELECT Player, OPS FROM c LIMIT 10`처럼 최상위 `LIMIT`에 `ORDER BY`가 없으면 "Top 10"에 아무 열 명이 돌아오고 실행마다 명단이 바뀐다. 모든 값이 진짜 값이라 천장도 차등도 불변식도 침묵하는 **명단만 틀린** 축이고, `_inject_tiebreak`도 못 쓴다 — 붙일 `ORDER BY`가 없다.

**그런데 이 모양은 한 번도 관측된 적이 없다.** app.log에는 검증에 실패한 SQL만 남고 이 쿼리는 문법상 깨끗해서 통과하므로, "일어나지 않는다"를 지금 가진 것으로는 증명할 수 없다. 근거 없이 거부하는 가드를 놓는 것이 정상 차트를 에러 메시지로 바꾸는 길이므로 — 같은 시기에 반올림 정렬 린트를 스코프 너머로 확장해봤더니 실제 SQL 21건 중 14건을 거부했다 — **로그만 남기고 통과시킨다.** 그 줄이 곧 측정이고, 세어서 0이면 짓지 않는다. 계측 후 첫 harness 153롤에서 0건.
...
> **차등 대조가 어느 스탯까지 실제로 잡는지는 `DIFFERENTIAL_COVERAGE.md`에 실측 매트릭스로 있다** — 통산/시즌별 × rate·+stat·카운팅, 그리고 커버 안 되는 것(ISO·WHIP·BABIP, 카운팅 미달, 시즌별 다행 선수의 카운팅, 별칭이 어긋난 컬럼). 스탯 목록이나 매칭 규칙을 바꾸면 그 문서의 프로브를 다시 돌려 갱신한다.
...
- 넷 다 아니면 → **그대로 응답** ✅ (5,000행 이하면 캐시됨 — 진입부 4번)
- 걸린 것이 **값 문제인데 결과가 5,000행(`MAX_CHART_ROWS`)을 넘으면** → **재요청 없이 즉시 거부** (행 수 안내). 재요청이 아무리 잘 나와도 같은 9,703행을 돌려주고 `_charted`가 어차피 거부하므로, 이 한 번의 호출은 결과와 무관하게 버려진다. 판단 근거인 행 수는 이미 `df0`에 있다. (아래 "넓은 질문은 재요청을 사지 않는다")
- 하나라도 걸리면 → **재요청 1회**. 무엇을 알려줄지는 트리거마다 다르다:
    ...
    - **불가능 rate** → **질문만** 다시 던진다. 상한 검사일 뿐이라 재검증 수단이 없다 (아래 "재요청에 무엇을 알려주나").
...
실제 원인은 **프롬프트가 그 자리에 헷갈리는 짝을 두었느냐**다. 이 실측에서 확인된 세 건이 전부 같은 모양이다:
...
### 11. 동명이인 경로 — 150/150, 이 실측에서 유일하게 결함이 없던 축
...
**왜 여기만 깨끗한지가 요점이다.** 이 경로는 LLM에 거의 의존하지 않는다 — 감지·해소·필터 주입이 전부 결정론적 코드이고, 모델이 하는 일은 `team_hints`를 채우는 것 하나뿐이다(그마저 100/100). 다른 축에서 찾은 결함(9이닝당 100% 실패, 가드 28% 스킵, 중간 CTE 37%, 손잡이 8/10, 스위치 9/10)은 전부 **프롬프트 규칙에 맡긴 부분**에서 나왔다. 이 대비가 이 프로젝트의 방향을 요약한다.
...
**가설 두 개가 반증됐다.** 처음엔 "매크로가 덮은 영역은 결함 0건, 못 덮는 +stat에서 결함이 전부 나왔다"는 상관을 근거로 삼았는데, 단순 rate A/B가 그 상관에 인과가 없음을 보였다. 다음엔 "컴패니언이 붙으면 6000타석 때와 같은 혼동이 재현될 것"으로 좁혔는데, 그것도 재현되지 않았다.
...
따라서 "1종 수렴이 회귀 탐지를 돕는다"도 좁혀 읽어야 한다 — 결과가 어차피 같으므로 결과 기반 탐지엔 차이가 없고, **SQL 텍스트를 신호로 쓸 때만** 이점이 있다(중간 CTE 36건 → 0건을 그 방식으로 확인했다). 사용자가 보는 차트에는 아무 영향이 없다.

**그래서 매크로를 어떻게 읽어야 하나.** 2026-08-06 시점의 이 모델(gemini-2.5-flash)은 단순 rate 공식을 안 틀린다. 매크로는 오류 **확률을 낮추는** 게 아니라 오류 **가능성을 제거**한다 — 그 차이는 지금 이 모델에서는 보이지 않는다. 값어치가 남는 근거는 셋이고, 전부 이 실측 밖이다: (1) 폴백 티어의 더 작은 모델은 다시 틀릴 수 있고, (2) 이력에 실제로 일어난 사례가 있으며(6000타석 건은 컴패니언 OPS를 매크로로 바꿔서 고쳤다), (3) 분산 수렴이 회귀 탐지를 돕는다. **보험이다 — 이 실측에서 프리미엄이 0으로 측정됐다고 무의미한 것은 아니지만, "매크로가 오류를 X% 줄인다"고 말할 근거는 없다.**
...
|문구|위치|무엇|
|---|---|---|
|`Cleaned SQL literals for %r`|`app.py`의 `_clean_literals`|A의 요약 (2026-08-05 이후)|
|`Repaired literals for %r`|같은 곳, **개명 전 문구**|위와 같은 이벤트|
|`Repaired corrupted SQL literals: %s`|`queries.py`의 `clean_corrupted_literals`|**별개 하위 이벤트** — 학교명 별칭 교정(`'서림초' → ['광주서림초','인천서림초']`). 개명된 적 없고 지금도 이 이름이다|
...
프로덕션은 pytest를 안 돌리므로 `-v` 절 없이도 깨끗하다. 로컬에서 **2026-08-06 이전** 로그를 셀 때만 `F`가 필요하다 (그 날짜에 `DropTestRuns`를 붙였다).
...
`A 리터럴 수리`는 `app.py`가 남기는 **요약 한 줄**만 센다. 학교명 별칭 교정 (`Repaired corrupted SQL literals`)은 질문이 안 찍혀 픽스처를 걸러낼 수 없으므로 따로 본다 — `grep -c "Repaired corrupted SQL literals"`.
